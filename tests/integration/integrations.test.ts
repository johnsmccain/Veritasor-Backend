...
function createMockIntegrationsRouter() {
  ...
  router.get('/connected', requireAuth, (_req: express.Request, res: express.Response) => {
    const businessId = res.locals.businessId as string

    const connected = connectedIntegrationsStore
      .filter((conn) => conn.businessId === businessId && conn.status === 'active')
      .map((conn) => ({
        id: conn.id,
  ...
  router.post('/:integrationId/connect', requireAuth, (req: express.Request, res: express.Response) => {
    ...
    const redirectUri = req.body.redirectUri || 'http://localhost:3000/integrations/callback'
  ...
  router.post('/callback', requireAuth, (req: express.Request, res: express.Response) => {
    ...
    const integration = availableIntegrations.find((i) => i.id === stateEntry.integrationId)
  ...
  router.delete('/:connectionId', requireAuth, (req: express.Request, res: express.Response) => {
...
describe('GET /api/integrations/available', () => {
  it('should list all available integrations without authentication', async () => {
    ...
    const stripeIntegration = response.body.integrations.find((i: { id: string }) => i.id === 'stripe')
...
describe('GET /api/integrations/connected', () => {
  ...
  it('should list connected integrations for authenticated business', async () => {
    ...
    expect(response.body.integrations[0]).toHaveProperty('integrationId', 'stripe')
    expect(response.body.integrations[0]).toHaveProperty('provider', 'stripe')
    expect(response.body.integrations[0]).toHaveProperty('accountId', 'acct_123')
...
describe('Integrations flow integration', () => {
  ...
  it('should handle multiple integrations for same business', async () => {
    ...
    const providers = connectedResponse.body.integrations.map((i: { provider: string }) => i.provider)
